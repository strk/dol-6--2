      ...
      if (map.layer_styles !== undefined && map.layer_styles[layername]) {
        ...
        for (var propname in style) {
          if (typeof style[propname] == 'object') {
            ...
            var plugin_context = new plugin_class(style[propname]['conf']);

            // Add plugin context functions to global context
            for (var key in plugin_context) {
              var newkey = plugin_name + '_' + propname + '_' + key;
              newContext[newkey] = OpenLayers.Function.bind(plugin_context[key],
                plugin_context); 
            }
        ...
        for (var plugin_name in style.plugins)
        {
              ...
              for (var key in plugin_context) {
                ...
                newContext[newkey] = OpenLayers.Function.bind(
                  plugin_context[key], plugin_context); // plugin_method_scope);
              }
        }
